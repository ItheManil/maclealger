...
const FaqSection = () => {
  ...
  return (
    <section className="py-[90px] px-[5%] text-center" id="faq">
      <span className="reveal block text-[11px] font-medium text-[var(--gold)] tracking-[0.1em] uppercase mb-4 text-center">Foire aux questions</span>
      <h2 className="reveal font-heading text-[clamp(28px,4vw,44px)] font-bold leading-[1.15] tracking-tight text-[var(--sand)] mb-2 mx-auto">Questions fréquentes</h2>
      <div className="max-w-[680px] mx-auto mt-14">
        {faqs.map((faq, i) => (
          <div key={i} className="reveal border-b border-[rgba(0,51,38,0.2)] py-5 cursor-pointer text-left" onClick={() => setOpenIndex(openIndex === i ? null : i)}>
            <div className="flex justify-between items-center gap-3">
              <span className="text-[15px] font-medium text-[var(--sand)]">{faq.q}</span>
              <span className={`text-[var(--gold)] text-xl shrink-0 transition-transform duration-200 ${openIndex === i ? 'rotate-45' : ''}`}>+</span>
            </div>
            {openIndex === i && (
              <p className="text-sm text-[var(--sand-soft)] leading-relaxed mt-3">{faq.a}</p>
            )}
          </div>
        ))}
      </div>
