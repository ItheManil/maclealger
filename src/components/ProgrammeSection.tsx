...
const ProgrammeSection = () => (
  <section className="py-[90px] px-[5%] bg-white" id="programme">
    <span className="reveal block text-[11px] font-medium text-[var(--gold)] tracking-[0.1em] uppercase mb-4">Au programme</span>
    ...
    <div className="mt-14">
      {items.map((item, i) => (
        <div key={i} className={`reveal grid grid-cols-[90px_1fr] max-[600px]:grid-cols-[70px_1fr] gap-6 max-[600px]:gap-3.5 py-6 border-b border-[rgba(0,51,38,0.2)] items-start ${i === 0 ? 'border-t' : ''}`}>
          <div>
            <span className="font-heading text-[22px] font-bold text-[var(--gold)] pt-[3px] block">{item.time}</span>
            <span className="text-[11px] text-[var(--sand-soft)] font-light block">{item.dur}</span>
          </div>
          <div>
            <p className="text-[15px] font-medium text-[var(--sand)]">{item.title}</p>
            <p className="text-[13px] text-[var(--sand-soft)] mt-1">{item.desc}</p>
          </div>
        </div>
      ))}
    </div>
